...
const CANDIDATES: CandidateRow[] = [
  { id: 'c1', shortName: '衡远包装', fullName: '常州衡远包装材料有限公司', sources: [{ type: 'system', label: '系统推荐' }, { type: 'field', label: '跑楼补全' }], matchReason: '代发稳定 + 法人身份已补全，公私交叉验证通过', stage: 'approved', bizScore: 5, personScore: 5, updatedAgo: '10分钟前', locked: false, sampleId: 'sample-hengyuan' },
  ...
  { id: 'c6', shortName: '盛拓模组', fullName: '盛拓模组科技有限公司', sources: [{ type: 'system', label: '系统推荐' }, { type: 'filter', label: '新能源方案' }], matchReason: '二级供应商，桥接节点角色', stage: 'pre_credit', bizScore: 5, personScore: 3, updatedAgo: '3小时前', locked: false },
...
function ConfidenceDots({ score, label }: { score: number; label: string }) {
  return (
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <span className="text-[12px] text-[#4E5969] w-20 shrink-0">所属行业</span>
                    <select className="flex-1 h-8 rounded border border-[#E5E6EB] px-2 text-[12px] text-[#1D2129] bg-white"><option>请选择行业</option><option>新能源</option><option>储能</option><option>包装材料</option><option>物流</option></select>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-[12px] text-[#4E5969] w-20 shrink-0">成立年限</span>
                    <select className="flex-1 h-8 rounded border border-[#E5E6EB] px-2 text-[12px] text-[#1D2129] bg-white"><option>全部</option><option>≥ 3 年</option><option>≥ 5 年</option><option>≥ 10 年</option></select>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-[12px] text-[#4E5969] w-20 shrink-0">注册资本</span>
                    <select className="flex-1 h-8 rounded border border-[#E5E6EB] px-2 text-[12px] text-[#1D2129] bg-white"><option>全部</option><option>≥ 100 万</option><option>≥ 500 万</option><option>≥ 1000 万</option></select>
                  </div>
                </div>
              </div>
              <div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <span className="text-[12px] text-[#4E5969] w-20 shrink-0">本行关系</span>
                    <select className="flex-1 h-8 rounded border border-[#E5E6EB] px-2 text-[12px] text-[#1D2129] bg-white"><option>全部</option><option>代发工资客户</option><option>按揭客户</option><option>对公结算客户</option></select>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-[12px] text-[#4E5969] w-20 shrink-0">资质标签</span>
                    <select className="flex-1 h-8 rounded border border-[#E5E6EB] px-2 text-[12px] text-[#1D2129] bg-white"><option>全部</option><option>专精特新</option><option>高新技术企业</option><option>小巨人</option></select>
                  </div>
                </div>
              </div>
...
function ImportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const statusStyle = { ok: { icon: '🟢', label: '正常', cls: '' }, downgrade: { icon: '🟡', label: '降级', cls: '' }, locked: { icon: '🔒', label: '锁死', cls: '' }, blocked: { icon: '❌', label: '拦截', cls: 'bg-red-50' } };
                ...
                <table className="w-full text-[12px]">
                  <thead>
                    <tr className="bg-[#F7F8FA] border-b border-[#E5E6EB]">
                      {['企业名', '法人身份(脱敏)', '征信', '状态', '融合'].map(h => (
                        <th key={h} className="text-left px-3 py-2 font-medium text-[#86909C]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
          ...
          {step === 2 ? (
            <>
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" className="text-[12px] bg-[#1677FF] hover:bg-[#0E5FC2]" onClick={() => { onClose(); setStep(1); }}>融合入库 (18)</Button>
              </div>
            </>
          ) : (
    ...
    const onScroll = () => {
      for (const sec of sections) {
        ...
        if (ref) {
          ...
          if (rect.top >= containerRect.top - 20 && rect.top < containerRect.top + 200) {
    ...
    <div className="space-y-0">
      ...
      <div className="flex rounded-xl border border-[#E5E6EB] overflow-hidden bg-white" style={{ minHeight: 520 }}>
        ...
        <div className="w-[180px] shrink-0 border-r border-[#F2F3F5] bg-[#FAFBFC] py-4">
          {sections.map(sec => (
            <button key={sec} onClick={() => scrollTo(sec)} className={cn('w-full text-left px-4 py-2.5 text-[12px] transition-colors', activeSection === sec ? 'bg-[#1677FF] text-white font-semibold' : 'text-[#4E5969] hover:bg-[#F2F3F5]')}>
              ● {sec}
            </button>
          ))}
        </div>
        ...
        <div ref={contentRef} className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div ref={el => { sectionRefs.current['基础画像'] = el; }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: '企业简称', value: candidate.shortName },
                { label: '全称', value: candidate.fullName },
                { label: '行业', value: '新能源包装材料' },
                { label: '注册资本', value: '500 万' },
                { label: '成立年限', value: '6 年' },
                { label: '所在地区', value: '江苏常州' },
              ].map(item => (
                <div key={item.label} className="text-[12px]">
                  <span className="text-[#86909C]">{item.label}</span>
                  <div className="mt-0.5 text-[#1D2129] font-medium">{item.value}</div>
                </div>
              ))}
            </div>
            <div className="mt-4 flex items-center gap-4">
              <div><span className="text-[10px] text-[#86909C]">企业置信度</span><ConfidenceDots score={candidate.bizScore} label="企" /></div>
              <div><span className="text-[10px] text-[#86909C]">法人置信度</span><ConfidenceDots score={candidate.personScore} label="人" /></div>
            </div>
          </div>
            ...
            <div className="flex rounded-lg border border-[#E5E6EB] overflow-hidden">
              ...
              <div className="flex-1 p-4 space-y-2">
                ...
                {[{ l: '身份', v: '34岁 男' }, { l: '籍贯', v: '江苏常州' }, { l: '本行', v: '按揭客户' }, { l: '置信度', v: '' }].map(r => (
                  <div key={r.l} className="flex items-center justify-between text-[12px]">
                    <span className="text-[#86909C]">{r.l}</span>
                    {r.v ? <span className="text-[#1D2129]">{r.v}</span> : <ConfidenceDots score={candidate.personScore} label="人" />}
                  </div>
                ))}
              </div>
            </div>
                ...
                <div className="px-4 py-3 space-y-1.5 bg-white font-mono text-[11px]">
                  {section.items.map(item => (
                    <div key={item.text} className="flex items-start gap-2">
                      <span className="font-bold w-3 shrink-0" style={{ color: item.color }}>{item.sign}</span>
                      <span className="text-[#1D2129]">{item.text}</span>
                    </div>
                  ))}
                </div>
              </details>
          ...
          <div ref={el => { sectionRefs.current['AI判断'] = el; }}>
            ...
            <div className="rounded-lg border border-[#1677FF] bg-[#E8F3FF] p-4">
              ...
              <p className="text-[11px] text-[#4E5969] leading-5 mb-3">
                订单、物流、回款三流匹配度高，代发工资与社保连续，公私联动评分 85，综合置信度 92%。建议进入产品匹配阶段。
              </p>
...
interface CounterpartyNode {
  id: string; name: string; direction: 'upstream' | 'downstream' | 'service'; txCount: number; txAmount: string; months: number; cycleDays: string; concentration: string; keywords: string[]; status: 'stable' | 'attention' | 'new'; relationStrength: number; evidenceSources: string[]; isMainChain: boolean;
}
...
export default function CustomerPoolScene({ activeModule, onModuleChange }: { activeModule: string; onModuleChange: (id: string) => void }) {
  ...
  const renderContent = () => {
    switch (activeModule) {
      ...
      default:
        if (detailCandidate) {
          return <EnterpriseDetail candidate={detailCandidate} onBack={() => setDetailCandidate(null)} />;
        }
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { type: 'system' as SourceType, icon: <Bot size={16} className="text-[#1677FF]" />, title: '系统推荐', main: `新增 ${stats.systemNew}`, sub: stats.systemSub, count: stats.system, highlight: true },
                { type: 'filter' as SourceType, icon: <Search size={16} className="text-[#1677FF]" />, title: '条件筛选', main: `命中 ${stats.filter + 120}`, sub: stats.filterSub, count: stats.filter, highlight: false },
                { type: 'field' as SourceType, icon: <UserPlus size={16} className="text-[#1677FF]" />, title: '外勤录入', main: `录入 ${stats.field + 23}`, sub: stats.fieldSub, count: stats.field, highlight: false },
              ].map(card => (
                <button key={card.type} onClick={() => { setSourceFilter(prev => prev === card.type ? 'all' : card.type); setPage(1); }} className={cn('rounded-xl border bg-white p-4 text-left transition-all hover:shadow-md', sourceFilter === card.type ? 'border-[#1677FF] ring-1 ring-[#1677FF]/20' : 'border-[#E5E6EB]')}>
                  <div className="flex items-center gap-2 mb-2">
                    {card.icon}
                    <span className="text-[13px] font-semibold text-[#1D2129]">{card.title}</span>
                    {card.highlight && <span className="w-2 h-2 rounded-full bg-[#F53F3F]" />}
                  </div>
            ...
            <div className="flex items-center justify-between gap-3 rounded-xl border border-[#E5E6EB] bg-white px-4 py-2.5 shadow-inner">
              <div className="flex items-center gap-2">
                <select value={sourceFilter} onChange={e => { setSourceFilter(e.target.value as any); setPage(1); }} className="h-8 rounded border border-[#E5E6EB] px-2 text-[12px] text-[#1D2129] bg-white">
                  <option value="all">来源: 全部</option><option value="system">系统推荐</option><option value="filter">条件筛选</option><option value="field">外勤录入</option>
                </select>
                <select value={stageFilter} onChange={e => { setStageFilter(e.target.value as any); setPage(1); }} className="h-8 rounded border border-[#E5E6EB] px-2 text-[12px] text-[#1D2129] bg-white">
                  <option value="all">阶段: 全部</option><option value="identified">已识别</option><option value="pre_credit">预授信</option><option value="approved">已批准</option><option value="vetoed">一票否决</option>
                </select>
                <div className="relative">
                  <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#C9CDD4]" size={13} />
                  <Input placeholder="搜索企业名称..." className="pl-8 h-8 w-48 text-[12px] bg-white border-[#E5E6EB]" />
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" className="h-8 text-[12px] text-[#4E5969] border-[#E5E6EB] gap-1" onClick={() => setFilterOpen(true)}>
                  <SlidersHorizontal size={12} />高级筛选
                </Button>
                <Button variant="outline" size="sm" className="h-8 text-[12px] text-[#1677FF] border-[#BEDAFF] gap-1" onClick={() => setImportOpen(true)}>
                  <Upload size={12} />导入名单
                </Button>
              </div>
            </div>
                    ...
                    return (
                      <TableRow key={c.id} className={cn('cursor-pointer transition-colors group', isLocked ? 'text-[#A9AEB8]' : 'hover:bg-[#F7F8FA]', c.sampleId === selectedSampleId && 'bg-[#E8F3FF] hover:bg-[#E8F3FF]')} onClick={() => { if (c.sampleId) selectSample(c.sampleId); setDetailCandidate(c); }}>
                        ...
                        <TableCell className="py-2.5">
                          <div className={cn('text-[13px] font-medium', isLocked ? 'text-[#A9AEB8]' : 'text-[#1D2129]')}>{c.shortName}</div>
                        ...
                        <TableCell className="py-2.5">
                          <div className="flex flex-wrap gap-1">
                            {c.sources.map(s => <SourceTag key={s.label} type={s.type} label={s.label} />)}
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="py-2.5">
                          <div className="space-y-0.5">
                            ...
                            <ConfidenceDots score={c.personScore} label="人" />
                          </div>
                        </TableCell>
                        <TableCell className="py-2.5 pr-4 text-right">
                          <div className={cn('text-[11px]', isLocked ? 'text-[#C9CDD4]' : 'text-[#86909C]')}>{c.updatedAgo}</div>
                          {isLocked && (
                            <div className="mt-1" title={c.lockReason}>
                              <Button variant="ghost" size="sm" className="h-5 text-[9px] px-1.5 text-[#A9AEB8] cursor-not-allowed" disabled>推进</Button>
                            </div>
                          )}
                        </TableCell>
                      </TableRow>
      ...
      case 'long-tail':
        return (
          <div className="rounded-xl border border-[#E5E6EB] bg-white p-8 text-center text-[#86909C] text-[13px]">
            该模块内容保持不变，请在左侧导航选择「候选资产池」或「关系图谱」查看重构后的页面。
          </div>
        );
    }
  };
